...
time.sleep(2.0)  # Waktu untk persiapan camera
...
while True:
    ...
    frame = imutils.resize(frame, width=500)  # mengubah ukuran gambar mjd 500
    rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    
    # Deteksi wajah
    boxes = cv2.CascadeClassifier(cv2.data.haarcascades + "haarcascade_frontalface_default.xml")#mendekteksi wjah
    ...
    for (x, y, w, h) in faces:
        cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 255, 0), 2) #menggambar kotk hijau
        save_face(frame, (y, x+w, y+h, x), name)  # memotong & menyimpan wajah
...
# Tutup proses
vs.release() #untk menghentikan akses ke webcam
cv2.destroyAllWindows() #menutup semua window openSV
